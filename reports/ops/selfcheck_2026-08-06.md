# Ops Self-Check — 2026-08-06 (Thu)

Scheduled weekday health check. Local files only; no external data fetched.

## Verdict: ⚠️ PROBLEM (recurring, unchanged) — scheduled pipeline still broken; pipeline has now not run for 31 days

Root cause is identical to every prior flagged check and still unaddressed. No new failure mode, no data loss. Stable-but-stalled, not deteriorating. One additional launchd fire since the 08-05 check (Wed 08-05 evening).
...
`results/launchd.err.log`, last modified **2026-08-05 18:00**, now holds **23** identical lines:
```
/bin/bash: .../code/run_auto.sh: Operation not permitted
```
One more than the 08-05 check (22 → 23), consistent with a single launchd fire on Wed 08-05 evening. launchd keeps firing on the weekday schedule but macOS denies execution of `run_auto.sh`. Cause unchanged: the script lives under the TCC-protected Desktop folder and the process launchd uses (`/bin/bash` / the launchd agent) lacks Full Disk Access — a permission block, not a missing execute bit (`run_auto.sh` is `-rwx------`, execute bit is present).

### 2. ⚠️ No successful full run since 2026-07-06 (31 days)
...
Of the last 20 real run_log rows before the stall, 18 are `fetch_trends_ticker, failed` with HTTP **429**. Trends stage was throttled by Google on Jul 6 and contributed to that run dying without `run_end`. Known limitation; not the scheduled pipeline blocker.
...
## Alert
Problem is recurring and still unaddressed at 31 days — short Korean status message recorded in the run log, kept brief and calm since it repeats a known, unchanged issue.
