# Ops Self-Check — 2026-07-15 (Wed)

Scheduled weekday health check. Local files only; no external data fetched.

## Verdict: ⚠️ PROBLEM (actionable, escalating) — scheduled pipeline still broken; pipeline has now not run for 9 days

Seventh flagged weekday (07-07, 07-08, 07-09, 07-10, 07-13, 07-14, 07-15; weekends not scheduled). The launchd job last fired **2026-07-14 18:14** and failed again with `Operation not permitted`, so still no new pipeline run. The stall that crossed the 2-day threshold on 07-08 is now at 9 calendar days. Root cause is unchanged and still unaddressed.

## What was checked

### 1. results/run_log.csv — ⚠️ pipeline stalled (9 days)
- Last `run_start`: **2026-07-06T08:13:47Z** (INCREMENTAL UPDATE). No new run 07-07 through 07-15.
- Last complete `run_end`: **2026-07-06T07:52:07Z**. → 9 days with no successful full run; staleness threshold far exceeded.
...
- Modified **2026-07-14 18:14**; now contains **seven** identical lines:
  `code/run_auto.sh: Operation not permitted`
- launchd fired on 07-07, 07-08, 07-09, 07-10, 07-13, and 07-14 and could **not execute** `run_auto.sh`. The script carries the execute bit, so this is a macOS TCC / Full Disk Access block on running a script inside the protected Desktop location — not a missing chmod.
...
- No new scheduled pipeline output since 07-06 confirms the launchd job has not successfully run for 9 days.
...
## Alert sent
Short Korean message recorded in the run log (problem is actionable, recurring, now at 9 days / 7th flagged weekday).
