# Ops Self-Check — 2026-08-03 (Mon)

Scheduled weekday health check. Local files only; no external data fetched.

## Verdict: ⚠️ PROBLEM (recurring, unchanged) — scheduled pipeline still broken; pipeline has now not run for 28 days

Root cause is identical to every prior flagged check and still unaddressed. No new failure mode, no data loss. Stable-but-stalled, not deteriorating. First monitor pass since Fri 07-31 (weekend gap is expected — weekday-only schedule).
...
`results/launchd.err.log`, last modified **2026-07-31 18:06**, now holds **20** identical lines:
```
/bin/bash: .../code/run_auto.sh: Operation not permitted
```
One more than the 07-31 morning check (19 → 20), consistent with a single launchd fire on Fri 07-31 evening. No new lines over the weekend, as expected for a weekday-only schedule. launchd keeps firing but macOS denies execution of `run_auto.sh`. Most likely cause unchanged: the script lives under the TCC-protected Desktop folder and the process launchd uses (`/bin/bash` / the launchd agent) lacks Full Disk Access — a permission block, not a missing execute bit.

### 2. ⚠️ No successful full run since 2026-07-06 (28 days)
...
prices: **502** CSVs | filings_8k: **303** CSVs (+1 non-CSV `text/` dir → 304 total entries, matching prior "304" counts) | trends: **46** CSVs. Unchanged from prior checks. Trends still capped at the 46-ticker seed by the 429 limit. No sharp coverage drop.
...
## Alert
Problem is recurring and still unaddressed at 28 days — short Korean status message recorded in the run log, kept brief and calm since it repeats a known, unchanged issue.
