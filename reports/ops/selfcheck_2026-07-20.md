# Ops Self-Check — 2026-07-20 (Mon)

Scheduled weekday health check. Local files only; no external data fetched.

## Verdict: ⚠️ PROBLEM (recurring, unchanged) — scheduled pipeline still broken; pipeline has now not run for 14 days

Ninth flagged weekday (07-07, 07-08, 07-09, 07-10, 07-13, 07-14, 07-15, 07-16, 07-17, then weekend gap, now 07-20). Root cause is unchanged and still unaddressed. No new failure mode and no data loss since the last check — the situation is stable-but-stalled, not deteriorating.
...
`results/launchd.err.log`, last modified **2026-07-17 18:00**, now holds **10** identical lines:
```
/bin/bash: .../code/run_auto.sh: Operation not permitted
```
One more than the 07-17 morning check (9 → 10), consistent with a single launchd fire on Fri 07-17 evening; no lines added over the weekend (job is weekday-only). launchd keeps firing on schedule but macOS denies execution of `run_auto.sh`. Most likely cause: the script lives under the TCC-protected Desktop folder and the process launchd uses (`/bin/bash` / the launchd agent) lacks Full Disk Access — this is a permission block, not a missing execute bit.

### 2. ⚠️ No successful full run since 2026-07-06 (14 days)
...
Last entry `2026-07-06 16:13:59 scheduled pipeline done`. The `push FAILED / No configured push destination` line is a historical Jul 6 setup issue — a git remote **is** now configured (`origin → github.com/japark22/8-K-sentiment-Google-Trends-Signal-pipeline.git`), so this is not a current blocker.
...
## Alert
Problem is recurring and still unaddressed at 14 days — short Korean status message recorded in the run log (kept brief/calm since it repeats a known, unchanged issue).
