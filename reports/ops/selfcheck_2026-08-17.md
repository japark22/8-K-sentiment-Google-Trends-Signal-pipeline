# Ops Self-Check — 2026-08-17 (Mon)

Scheduled weekday health check. Local files only; no external data fetched.

## Verdict: ⚠️ PROBLEM (recurring, unchanged) — scheduled pipeline still broken; pipeline has now not completed a run for 42 days

Same root cause as every flagged check since early July, still unaddressed. No new failure mode, no data loss, no deterioration. Stable-but-stalled. One new launchd fire since the last check (Fri 08-14 18:00), which failed identically. Today's scheduled fire (Mon 08-17 18:00) has not happened yet at the time of this check.
...
- schedule/local.8ktrends.plist (schedule)
...
`results/launchd.err.log`, last modified **2026-08-14 18:00**, now holds **30** identical lines:
```
/bin/bash: .../code/run_auto.sh: Operation not permitted
```
One more than the 08-14 check (29 → 30), consistent with a single launchd fire on Fri 08-14 evening. Zero non-matching lines — no new error type. The plist schedule is confirmed as weekdays Mon–Fri 18:00 local, so launchd keeps firing correctly; macOS is denying execution of `run_auto.sh`. Cause unchanged: the script lives under the TCC-protected Desktop folder and the process launchd uses lacks Full Disk Access — a permission block, not a missing execute bit (`run_auto.sh` is `-rwx------`, execute bit present).

### 2. ⚠️ No successful full run since 2026-07-06 (42 days)
Last complete `run_end`: **2026-07-06T07:52:07Z** ("INCREMENTAL UPDATE (--only ic,sentiment,trends_factor)"). The final Jul 6 incremental `run_start` (08:13:47Z) reached only `build_universe` (success, 503 names / 503 CIKs) and never produced a matching `run_end` — it died during the trends stage. No `run_end` within the last ~2 days → pipeline effectively stopped. `run_log.csv` unchanged since 2026-07-06; latest row timestamp is 2026-07-06T08:13:48Z. Only 2 `run_end` rows total in the log.

### 3. Google Trends 429 rate-limiting on the last real run (secondary, pre-existing)
The last ~18 real run_log rows before the stall are `fetch_trends_ticker, failed` with HTTP **429** (UNH, ABBV, COST, INTC, WFC, QCOM, TXN, PM, DHR, VZ, CAT, INTU, GE, MMM, AES, APD, ALL, AEP). Trends was throttled by Google on Jul 6 and contributed to that run dying without `run_end`. Known limitation; not the scheduled pipeline blocker.

### 4. results/ic_history.csv — ✅ healthy (but stale)
3 rows, all 8k_sentiment: h1 = -0.001847 (n=1994), h3 = 0.020569 (n=1972), h5 = 0.009149 (n=1945). All |IC| well below 0.30. **No POSSIBLE_LOOKAHEAD flag.** Values unchanged since 07-06 (no new run has produced fresh IC).

### 5. results/pipeline.log — stale, no new content
Last entry `2026-07-06 16:13:59 scheduled pipeline done`, which also records a `push FAILED` ("No configured push destination"). No new activity; consistent with the pipeline not having run. A git remote IS now configured (`origin → github.com/japark22/8-K-sentiment-Google-Trends-Signal-pipeline.git`), so that Jul 6 push error predates the remote setup and is not a current active blocker; it will only re-verify once scheduled pipeline runs again. Latest commit: `c535958 Slim repo: untrack bulk raw data`.
...
## Alert
Problem is recurring and still unaddressed at 42 days — short, calm Korean status message recorded in the run log. Kept brief since it repeats a known, unchanged issue.
